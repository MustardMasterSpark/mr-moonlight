...
{
    internal sealed class EditorTabView : BindableElement, INotifyValueChanged<int>
    {
        private const string UssClassName = "flora-editor-tab-view";
        private const string TabHeaderClassName = "flora-editor-tab-view__tab-header";
        private const string TabContentClassName = "flora-editor-tab-view__tab-content";
        private const string TabClassName = "flora-editor-tab-view__tab";
        private const string ActiveTabClassName = "flora-editor-tab-view__tab--active";
        ...
        public int value
        {
            ...
            set
            {
                int normalizedValue = NormalizeTabIndex(value);
                if (m_Index == normalizedValue)
                    return;

                if (panel != null)
                {
                    using ChangeEvent<int> pooled = ChangeEvent<int>.GetPooled(m_Index, normalizedValue);
                    pooled.target = this;
                    SetValueWithoutNotify(normalizedValue);
                    SendEvent(pooled);
                }
                else
                    SetValueWithoutNotify(normalizedValue);
        ...
        {
            newValue = NormalizeTabIndex(newValue);
            if (newValue == -1)
            {
                ClearActiveTab();
                return;
        ...
        private int NormalizeTabIndex(int index)
        {
            ...
            return index >= 0 && index < m_Tabs.Count ? index : 0;
        }
