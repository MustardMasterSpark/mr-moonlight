...
// Copied from Packages/com.unity.visualeffectgraph/Shaders

#ifndef FLORA_NOISE_INCLUDED
#define FLORA_NOISE_INCLUDED
...
// Simplex Noise
float2 SimplexGrad2D(float2 p)           // unit-length gradient from a cell id
{
    // two inexpensive hashes →   [-0.5, 0.5]²  → normalized
    float gx = frac(sin(dot(p, float2(127.1f, 311.7f))) * 43758.5453f ) - 0.5f;
    float gy = frac(sin(dot(p, float2(269.5f, 183.3f))) * 43758.5453f ) - 0.5f;
    float2 g = float2(gx, gy);
    return normalize(g + 1e-6f);          // avoid 0-length vectors
}

float3 SimplexGrad3D(float3 p)           // unit-length gradient on the sphere
{
    float3 q = frac(sin(p.yzx * float3(127.1f, 311.7f, 74.7f)
                      + p.zxy * float3(269.5f, 183.3f, 246.1f)
                      + p.xyz * float3(419.2f, 371.9f, 133.1f)) * 43758.5453f ) - 0.5f;

    return normalize(q + 1e-6f);
}

float2 GenerateSimplexNoise1D(float x)
{
    float i0 = floor(x);
    float x0 = x - i0;
    float x1 = x0 - 1.0f;

    float2 h  = NoiseHash1D(i0);
    float g0  = (h.x < 0.5f) ?  1.0f : -1.0f;
    float g1  = (h.y < 0.5f) ?  1.0f : -1.0f;

    float t0 = 1.0f - x0 * x0;
    float t1 = 1.0f - x1 * x1;
    float n0 = 0.0f, n1 = 0.0f;
    float d  = 0.0f;                       // derivative
    ...
    if (t1 > 0.0f)
    {
        ...
        float t4  = t2 * t2;
        n1        = t4 * g1 * x1;
        d        += t4 * g1 + (-8.0f * t2 * t1 * x1) * g1 * x1;
    }

    // normalisation (≈ 0.395 = 2.0 / (sqrt(5) + 1))
    const float NORM = 0.395f;
    return float2( (n0 + n1) * NORM, d * NORM );
}

float3 GenerateSimplexNoise2D(float2 v)
{
    const float F2 = 0.3660254037844386f;   // (√3-1)/2
    const float G2 = 0.2113248654051871f;   // (3-√3)/6

    float2 s  = v + dot(v, float2(F2, F2));
    float2 i  = floor(s);
    float2 t  = i - dot(i, float2(G2, G2));
    float2 x0 = v - t;

    float2 i1 = (x0.x > x0.y) ? float2(1.0f, 0.0f) : float2(0.0f, 1.0f);
    float2 x1 = x0 - i1 + float2(G2, G2);
    float2 x2 = x0 - 1.0f + float2(2.0f * G2, 2.0f * G2);

    float2 g0 = SimplexGrad2D(i);
    float2 g1 = SimplexGrad2D(i + i1);
    float2 g2 = SimplexGrad2D(i + 1.0f);
    ...
    {
        float2  x  = (k == 0) ? x0 : ((k == 1) ? x1 : x2);
        float2  g  = (k == 0) ? g0 : ((k == 1) ? g1 : g2);
        float   t0 = 0.5f - dot(x, x);
        if (t0 > 0.0f)
        {
            float t2 = t0 * t0;
            float t4 = t2 * t2;
            ...
            n += t4 * gv;
            d += t4 * g + (-8.0f * t2 * t0 * gv) * x;
        }
    }

    // normalisation constant for 2-D simplex = 70
    const float NORM = 70.0f;
    return float3( n * NORM, d * NORM );
}

// ───────────────────────────── 3-D simplex noise ───────────────────────────────
float4 GenerateSimplexNoise3D(float3 v)
{
    const float F3 = 1.0f / 3.0f;          //  (√3-1)/3   … but using 1/3 keeps the same skew
    const float G3 = 1.0f / 6.0f;          //  (3-√3)/6

    float3 s  = v + dot(v, float3(F3, F3, F3));
    float3 i  = floor(s);
    float3 t  = i - dot(i, float3(G3, G3, G3));
    float3 x0 = v - t;

    float3 g   = step(x0.yzx, x0.xyz);
    float3 l   = 1.0f - g;
    float3 i1  = min(g.xyz, l.zxy);        // Second corner
    float3 i2  = max(g.xyz, l.zxy);        // Third  corner

    float3 x1 = x0 - i1 + float3(G3, G3, G3);
    float3 x2 = x0 - i2 + float3(2.0f * G3, 2.0f * G3, 2.0f * G3);
    float3 x3 = x0 - 1.0f + float3(3.0f * G3, 3.0f * G3, 3.0f * G3);

    float3 g0 = SimplexGrad3D(i);
    float3 g1 = SimplexGrad3D(i + i1);
    float3 g2 = SimplexGrad3D(i + i2);
    float3 g3 = SimplexGrad3D(i + 1.0f);
    ...
    {
        float3  x = (k == 0) ? x0 : (k == 1 ? x1 : (k == 2 ? x2 : x3));
        float3  g = (k == 0) ? g0 : (k == 1 ? g1 : (k == 2 ? g2 : g3));
        float   t0 = 0.6f - dot(x, x);     // 0.6 = (3/4) √5  – keeps 4D deriv-generalisation; works fine
        if (t0 > 0.0f)
        {
            float t2 = t0 * t0;
            float t4 = t2 * t2;
    ...
    // normalisation constant for 3-D simplex = 32
    const float NORM = 32.0f;
    return float4( n * NORM, d * NORM );
}

NOISE_TEMPLATE(Simplex, float,  float2, GenerateSimplexNoise1D);
NOISE_TEMPLATE(Simplex, float2, float3, GenerateSimplexNoise2D);
NOISE_TEMPLATE(Simplex, float3, float4, GenerateSimplexNoise3D);

CURL_NOISE_2D_TEMPLATE(Simplex);
CURL_NOISE_3D_TEMPLATE(Simplex);
