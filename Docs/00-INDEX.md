...
| `lighting.md` | **Anything that is a light or decides how light looks.** The single reference: sun / day-night presets, HAZE fog, sky, flashlight, the hands' lighting, enemy lamps, flares, Light Layers, every lighting tunable (and which are unwired), all the traps. Update it whenever you touch a light | 🔴 **Read first for lighting** |
| `viewmodel-light-layers.md` | **Before adding any light or any first-person mesh.** The `ViewModel` URP Light Layer: why the hands are excluded from the flashlight and personal light, who applies it, how to tune the hands' fill light | 🔴 High |
...
| `character-pipeline-guide.md` | **Any character, start to finish.** The nine numbered stages from "about to model" to "in the game holding a weapon" — who does each one, what to hand over, and the exact phrase to say to resume at any stage. Wraps wizard §4 and owns the player-only viewmodel-arms and visible-body stages | 🔴 **Read first for characters** |
| `tracey-rig-strategy.md` | **Why** the player carries two skeletons that never merge, why weapon animations can't be retargeted onto a body, the AccuRig↔vendor arm bone map, and the 2026-09-05 build record with three silent traps | 🔴 High (with the guide above) |
...
| `dual-project-workflow.md` | Working with Playground (`E:\playground\Playground`), the sandbox project for testing new/bulk-imported assets before they enter Mr. Moonlight — how the two MCP bridges are kept separate, and how assets actually move between the two | 🟡 Reference |
