...
namespace PampelGames.Shared.Tools.PGInspector
{
    public static class PGModuleEditorUtility
    {
        ...
        public const string PartsMenu = "partsMenu";
        public const string AddPartsMenu = "addPartsMenu";
        ...
        /// <summary>
        ///     Creates the main toolbar, used to add and remove modules.
        /// </summary>
        public static Toolbar CreateMainToolbar()
        {
            return CreateMainToolbarInternal();
        }
        ...
        private static Toolbar CreateMainToolbarInternal()
        {
            var mainToolbar = new Toolbar();
            mainToolbar.name = Toolbar;
            mainToolbar.style.height = new StyleLength(Length.Auto());
            mainToolbar.style.alignItems = new StyleEnum<Align>(Align.Stretch);
            mainToolbar.style.justifyContent = new StyleEnum<Justify>(Justify.SpaceBetween);
            mainToolbar.PGBorderWidth(1);

            var leftSide = new VisualElement();
            leftSide.name = LeftSide;
            leftSide.style.flexDirection = new StyleEnum<FlexDirection>(FlexDirection.Row);
            mainToolbar.Add(leftSide);

            var rightSide = new VisualElement();
            rightSide.name = RightSide;
            rightSide.style.flexDirection = new StyleEnum<FlexDirection>(FlexDirection.RowReverse);
            mainToolbar.Add(rightSide);

            var partsMenu = new ToolbarMenu();
            partsMenu.name = PartsMenu;
            partsMenu.PGBorderWidth(1, 0, 0, 0);
            rightSide.Add(partsMenu);

            var addPartsMenu = new ToolbarMenu();
            addPartsMenu.name = AddPartsMenu;
            addPartsMenu.style.width = 33;
            addPartsMenu.PGMargin(0);
            addPartsMenu.PGBoldText();
            addPartsMenu.text = "+";
            addPartsMenu.tooltip = "Add a new item.";
            addPartsMenu.style.fontSize = 21;
            addPartsMenu.style.unityTextAlign = new StyleEnum<TextAnchor>(TextAnchor.MiddleCenter);
            addPartsMenu.PGBorderWidth(1, 1, 0, 0);
            addPartsMenu.PGRemoveMenuArrow(true, false);
            rightSide.Add(addPartsMenu);
        ...
        private static VisualElement CreateItemParentInternal(PGIModule module, int i, bool activeButton, bool chanceSlider)
        {
            ...
            if (activeButton)
            {
                var moduleActive = new ToolbarButton();
                ...
                var moduleActiveImage = new VisualElement();
