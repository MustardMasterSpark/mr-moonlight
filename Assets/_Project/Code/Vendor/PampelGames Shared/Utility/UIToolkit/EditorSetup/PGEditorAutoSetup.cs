...
namespace PampelGames.Shared.Utility
{
    public static class PGEditorAutoSetup
    {
            ...
            VisualElement parentElement)
        {
            ...
            var fields = GetFieldsOrderedByBase<T>();
            ...
            foreach (var field in fields)
            {
                ...
                var pgDropdownBox = field.GetCustomAttribute<PGDropboxAttribute>();
                var isDropdownBox = pgDropdownBox != null;
                ...
                else if (field.FieldType == typeof(string) && isDropdownBox)
                {
                    var dropbox = new VisualElement();
                    var tooltipAttr = field.GetCustomAttribute<TooltipAttribute>();
                    if (tooltipAttr != null) dropbox.tooltip = tooltipAttr.tooltip;

                    dropbox.style.height = pgDropdownBox.Height;
                    dropbox.style.backgroundColor = PGColors.InputFieldBackground();
                    dropbox.PGBorderWidth(2f);
                    dropbox.PGBorderRadius(2f);
                    dropbox.PGBorderColor(PGColors.CustomBorder());
                    dropbox.style.alignContent = Align.Center;
                    dropbox.style.alignItems = Align.Center;
                    dropbox.style.justifyContent = Justify.Center;

                    var methodName = pgDropdownBox.DropText;
                    if (string.IsNullOrEmpty(methodName)) methodName = pgDropdownBox.MethodName;
                    var dropboxLabel = new Label(CreateLabelString(methodName));
                    dropboxLabel.PGBoldText();
                    dropbox.Add(dropboxLabel);

                    dropbox.RegisterCallback<DragUpdatedEvent>(evt => { DragAndDrop.visualMode = DragAndDropVisualMode.Copy; });
                    ...
                    dropbox.RegisterCallback<DragPerformEvent>(evt =>
                    {
                        ...
                        if (droppedObjects.Count > 0)
                        {
                            var target = isSerializedObject ? serializedObject.targetObject : property.serializedObject.targetObject;
                            var method = target.GetType().GetMethod(methodName, BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance,
                                null, new[] {typeof(List<GameObject>)}, null);
                            method?.Invoke(target, new object[] {droppedObjects});
                        }
                    });
                    AddToParent(dropbox);
                }
        ...
        public static FieldInfo[] GetFieldsOrderedByBase<T>()
        {
