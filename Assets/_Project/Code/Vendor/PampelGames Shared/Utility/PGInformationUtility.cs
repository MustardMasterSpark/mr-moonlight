...
namespace PampelGames.Shared.Utility
{
    public static class PGInformationUtility
    {
        ...
        public static List<GameObject> CreateSpheres(Vector3[] vertices, float scaleMultiplier = 0.01f)
        {
            var spheres = new List<GameObject>();
            for (var i = 0; i < vertices.Length; i++)
            {
                var vertex = vertices[i];
                var sphere = CreateSphere(vertex, scaleMultiplier);
                sphere.name = i + "";
                spheres.Add(sphere);
            }
        ...
        public static void DrawBox(Vector3 position, float size = 1f, Color color = default)
        {
            ...
            var halfExtents = new Vector3(size * 0.5f, size * 0.5f, size * 0.5f);
            var frontTopLeft = position + new Vector3(-halfExtents.x, halfExtents.y, -halfExtents.z);
            var frontTopRight = position + new Vector3(halfExtents.x, halfExtents.y, -halfExtents.z);
            var frontBottomLeft = position + new Vector3(-halfExtents.x, -halfExtents.y, -halfExtents.z);
            var frontBottomRight = position + new Vector3(halfExtents.x, -halfExtents.y, -halfExtents.z);
            var backTopLeft = position + new Vector3(-halfExtents.x, halfExtents.y, halfExtents.z);
            var backTopRight = position + new Vector3(halfExtents.x, halfExtents.y, halfExtents.z);
            var backBottomLeft = position + new Vector3(-halfExtents.x, -halfExtents.y, halfExtents.z);
            var backBottomRight = position + new Vector3(halfExtents.x, -halfExtents.y, halfExtents.z);
