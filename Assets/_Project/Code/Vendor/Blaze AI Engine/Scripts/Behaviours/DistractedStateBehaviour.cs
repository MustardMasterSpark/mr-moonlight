...
namespace BlazeAISpace
{
    ...
    public class DistractedStateBehaviour : BlazeBehaviour
    {
        ...
        public override void Close()
        {
            ...
            if (blaze.navmeshAgent.isOnOffMeshLink) return;
        ...
        {   
            ...
            if (!turnedToLocation && !isIdle || !turnedToLocation && !checkLocation) 
            {
                ...
                if (_timeToReact >= timeToReact)
                {
                    ...
                    {
                        PlayIdleAnim(blaze.previousState);
                        turnedToLocation = true;
                        _timeToReact = 0f;
                    }
                }
                else {
                    // idle animation to play before turning
                    PlayIdleAnim(blaze.previousState);
                }
            }
        ...
        void PlayIdleAnim(BlazeAI.State previousState)
        {
            if (previousState == BlazeAI.State.normal)
            {
            ...
            {
                blaze.animManager.Play(alertStateBehaviour.idleAnim[0], checkAnimT);
            }
        }
        ...
        void EndSearchExit()
        {
            ...
            _endSearchTimer += Time.deltaTime;
            if (_endSearchTimer >= endSearchAnimTime)
            {
                ResetDistraction();
                ResetStayIdle();
                blaze.SetState(blaze.previousState);
