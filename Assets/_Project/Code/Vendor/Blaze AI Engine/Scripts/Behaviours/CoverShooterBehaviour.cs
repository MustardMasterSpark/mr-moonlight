...
namespace BlazeAISpace
{
    ...
    public class CoverShooterBehaviour : BlazeBehaviour
    {
        ...
        [Tooltip("The radius the AI will use to find the nearest navmesh point to it's target. This prevents the AI from thinking it's target is unreachable just because it stands on a small point with no navmesh. The best value is: navmesh agent height + navmesh agent radius.")]
        [Min(0)] public float navmeshSamplingRadius = 2.3f;
        ...
        void OnValidate()
        {
            ...
            if (moveForwards && moveBackwards)
            {
                ...
                {
                    moveBackwardsDistance = Mathf.Clamp(moveForwardsToDistance - 1, 0, moveForwardsToDistance - 1);
                }
            }
        ...
        void CheckHeightAndReachable()
        {
            ...
            float samplingRadius = blaze.navmeshAgent.height + blaze.navmeshAgent.radius;
            ...
            {   
                isReachable = blaze.ClosestNavMeshPoint(heightHit.point, samplingRadius, out closestEnemyPoint);
                return;
            }
        ...
        {   
            ...
            int hits = Physics.SphereCastNonAlloc(transform.position + blaze.vision.visionPosition, 0.15f, strafeSide, strafingHitArr, (blaze.navmeshAgent.radius * 2) + blaze.navmeshAgent.height/2, layersToHit);
            ...
            float rayDistance =  blaze.distanceToEnemy + 2;
            
            if (shouldAttackCover) 
            {
                if (enemyCover)
                {
                    if (!blaze.CheckTargetVisibleWithRay(enemyCover.transform, visibilityOffset, blaze.enemyColPoint - visibilityOffset, rayDistance, layersToHit) 
                        && !blaze.CheckTargetVisibleWithRay(blaze.enemyToAttack.transform, visibilityOffset, blaze.enemyColPoint - visibilityOffset, rayDistance, layersToHit)) {
                        ChangeStrafeDirection();
                        return;
                    }
                }
                else {
                    if (!blaze.CheckTargetVisibleWithRay(blaze.enemyToAttack.transform, visibilityOffset, blaze.enemyColPoint - visibilityOffset, rayDistance, layersToHit)) 
                    {
                        ChangeStrafeDirection();
                        return;
                    }
        ...
        public virtual void MoveBackwards(Vector3 target)
        {
            ...
            blaze.StrafeMove(-transform.forward, moveBackwardsSpeed, moveBackwardsAnim, forwardAndBackAnimT);
        ...
        public virtual void CallOthers()
        {
            ...
            for (int i=0; i<callOthersNum; i+=1) 
            {
                ...
                // if doesn't have default attack state scripts
