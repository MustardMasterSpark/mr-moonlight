...
namespace BlazeAISpace
{
    ...
    public class AttackStateBehaviour : BlazeBehaviour
    {
        ...
        [Tooltip("If enabled, the AI will not chase it's target if it gets out of vision. If disabled, the AI will chase.")]
        public bool avoidChase;
        [Tooltip("Set the amount of time you want the AI to keep being updated with the target's location when they're out of view. This helps the AI be smarter and more accurate during chases and corners. So for example: if it's set to 1 and the player suddenly goes out view then the AI will keep being updated with the target's location for 1 second, enabling the AI to chase the target more accurately.")]
        [Min(0)] public float targetLocationUpdateTime = 0.5f;
        
        [Tooltip("Will auto set the navmesh sampling radius used to find the nearest navmesh point to the target.")]
        public bool autoSetNavmeshSamplingRadius = true;
        [Tooltip("The radius the AI will use to find the nearest navmesh point to it's target. This prevents the AI from thinking it's target is unreachable just because it stands on a small point with no navmesh. The best value is: navmesh agent height + attack distance.")]
        [Min(0)] public float navmeshSamplingRadius = 2.5f;
        ...
        public bool ranged;
        [Min(0), Tooltip("The idle distance between the AI (waiting to attack) and the target. If distance between the AI and the target is more than this value, the AI will chase the player to close the distance.")]
        ...
        [Min(0), Tooltip("The extra distance the AI will tolerate if the target goes farther than the [Distance From Enemy]. This avoids the skating effect.")]
        public float distanceTolerance = 0.75f;
        ...
        [Min(0)] public float searchRadius = 4;
        [Tooltip("Will show as a white wire sphere Gizmo")]
        ...
        Vector3 currentStrafePoint = Vector3.zero;
        ...
        float distanceLock = -1f;
        float distanceLockCooldown = 0f;
        const float distanceLockDuration = 0.5f; // half a second
        ...
        public override void Main()
        {
            ...
            // if in companion mode and no SetTarget() called -> return to normal
            if (blaze.companionMode && blaze.checkEnemyPosition == Vector3.zero)
            {
            ...
            if (!blaze.companionMode && isSearching)
            {
                if (blaze.MoveTo(searchPointLocation, alertStateBehaviour.moveSpeed, alertStateBehaviour.turnSpeed, alertStateBehaviour.moveAnim)) 
                {
                    ...
                    else
                    {
                        if (!isLastSearchPoint)
                        {
                            isLastSearchPoint = true;
                            return;
                        }
                    }

                    // AI has went through all search points and is time to exit
                    EndSearchExit();
                    return;
                }
        ...
        void OnDrawGizmosSelected() 
        {
            ...
            if (searchLocationRadius && showSearchRadius) 
            {
                Gizmos.color = Color.white;
        ...
        void OnValidate()
        {   
            if (distanceFromEnemy < moveBackwardsDistance || moveBackwardsDistance >= distanceFromEnemy)
            {
                moveBackwardsDistance = Mathf.Clamp(distanceFromEnemy - 0.5f, 0, distanceFromEnemy - 0.5f);
            }
        ...
        public virtual void ReachableBehaviour(Transform target, float distance, float minDistance, float backupDist)
        {
            ...
            float realDist = Mathf.Sqrt(distance);
            float minDist = minDistance;
            float distanceToleranceToUse = distanceTolerance;

            // move backwards if too close
            if (realDist < backupDist)
            {
                distanceLock = -1f;
            ...
            // if attacking -> force move and reset distance lock
            if (blaze.isAttacking)
            {
                distanceToleranceToUse = 0.1f;
                if (realDist > minDist)
                {
                    distanceLock = -1f;
            ...
            if (distanceLockCooldown > 0f)
            {
                distanceLockCooldown -= Time.deltaTime;
            }

            if (distanceLock >= 0f)
            {
                float diff = Mathf.Abs(realDist - distanceLock);
                if (diff <= distanceToleranceToUse)
                {
                    if (blaze.isAttacking)
                    {
                        Attack();
            ...
            distanceLock = realDist;
            distanceLockCooldown = distanceLockDuration;
        ...
        void CheckHeightAndReachable()
        {
            ...
            float samplingRadius = blaze.navmeshAgent.height + attackDistance;
            if (!autoSetNavmeshSamplingRadius)
            {
                samplingRadius = navmeshSamplingRadius;
            }
            ...
            {
                Vector3 result;
                isReachable = blaze.ClosestNavMeshPoint(heightHit.point, samplingRadius, out result);
            ...
            {   
                Vector3 result;
                isReachable = blaze.ClosestNavMeshPoint(blaze.enemyColPoint, samplingRadius, out result);
                return;
            }
        ...
        public virtual void CallOthers()
        {
            ...
            for (int i=0; i<callOthersNum; i+=1) 
            {
                ...
                if (callOthersHitArr[i].transform.root == lastEnemy.root) continue;
                ...
                // if call isn't set to pass colliders then make sure there are no obstacles in between
                if (!callPassesColliders) 
                {
                    ...
                    if (Physics.Raycast(currentCenter, dir.normalized, out rayHit, distance, blaze.vision.layersToDetect))
                    {
                        if (!rayHit.transform.root != target.root) continue;
                    }
                }
                ...
                for (int x=1; x<6; x++)
                {
                    ...
                    if (!blaze.IsPathReachable(tempPos)) continue;
        ...
        public virtual void StrafeMovement(int direction)
        {
            ...
            Vector3 toEnemy = blaze.enemyColPoint - transform.position;
            Vector3 strafeSide = Vector3.Cross(toEnemy, Vector3.up).normalized;
            ...
            float sideDist = Mathf.Max(0, blaze.distanceToEnemy - 1f);

            if (!isStrafing || currentStrafePoint == Vector3.zero)
            {
                Vector3 basePoint = blaze.ValidateYPoint(transform.position);

                // Side offset
                Vector3 sideOffset = strafeSide * sideDist;

                // Final cached point
                currentStrafePoint = basePoint + sideOffset;
            }

            // check if point reachable and has navmesh every 4 frames
            if (strafeCheckPathElapsed >= 4)
            {
                ...
                Vector3 sidePointToCheck = transform.position + transform.right * blaze.navmeshAgent.height;
                if (direction == 0) 
                    sidePointToCheck = transform.position - transform.right * blaze.navmeshAgent.height;
                
                sidePointToCheck = sidePointToCheck + transform.forward * 0.4f;
                if (!blaze.IsPathReachable(sidePointToCheck))
                {
            ...
            System.Array.Clear(strafingHitArr, 0, strafingHitArr.Length);
            int hits = Physics.SphereCastNonAlloc(
                transform.position + blaze.vision.visionPosition, 
                0.15f, 
                strafeSide, 
                strafingHitArr, 
                (blaze.navmeshAgent.radius * 2) + blaze.navmeshAgent.height / 2, 
                layersToHit
            );
            ...
            Vector3 visibilityOffset = transform.TransformPoint(
                new Vector3(
                    (direction == 0 ? -(blaze.navmeshAgent.radius + 0.7f) : (blaze.navmeshAgent.radius + 0.7f)),
                    0f,
                    0f
                ) + blaze.vision.visionPosition
            );

            if (!blaze.CheckTargetVisibleWithRay(blaze.enemyToAttack.transform, visibilityOffset, blaze.enemyColPoint - visibilityOffset, blaze.distanceToEnemy + 2, layersToHit))
            {
                StopOrChangeStrafeDirection();
                return;
            }
            ...
            string anim = (direction == 0 ? leftStrafeAnim : rightStrafeAnim);
            blaze.RotateTo(blaze.enemyToAttack.transform.position, 20);
            blaze.StrafeMove(strafeSide, strafeSpeed, anim, strafeAnimT);
        ...
        public virtual void MoveBackwards(Vector3 target)
        {
            ...
            blaze.StrafeMove(-transform.forward, moveBackwardsSpeed, moveBackwardsAnim, moveBackwardsAnimT);
        ...
        {
            if (!isMovingBackwards || blaze.isAttacking) return true;
        ...
        void SetSearchPoint()
        {
            blaze.ClosestNavMeshPoint(blaze.RandomSpherePoint(blaze.enemyColPoint, searchRadius, false), 5, out searchPointLocation);
        ...
        public void EndSearchExit()
        {
            ...
            endSearchTimer += Time.deltaTime;
            if (endSearchTimer >= endSearchAnimTime)
            {
                idle = true;
                ReturnToAlert();
            }
