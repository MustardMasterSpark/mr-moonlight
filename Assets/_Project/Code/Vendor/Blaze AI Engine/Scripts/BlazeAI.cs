...
public class BlazeAI : MonoBehaviour
{
    ...
    public virtual void Start()
    {
        ...
        // set state on awake
        State stateToUse = State.normal;
        if (!useNormalStateOnAwake)
        {
            stateToUse = State.alert;
        }
        
        SetState(stateToUse);
        previousState = stateToUse;
    }
    ...
    void OnAnimatorMove()
    {
        ...
        Quaternion rootMotionRot = anim.deltaRotation;

        // Move agent using root motion
        navmeshAgent.nextPosition += rootMotionDelta;
    ...
    public void StrafeMove(Vector3 dir, float moveSpeed, string anim, float animT)
    {
        isStrafing = true;
        animManager.Play(anim, animT);
        ...
        navmeshAgent.Move(dir * moveSpeed * Time.deltaTime);
    }
    ...
    public virtual void VisionCheck()
    {
        ...
        {
            if (visionHitArr[i].transform.root == transform.root) continue;

            // if companion mode is on -> stop AI from targeting companion 
            if (companionMode && companionTo != null && companionTo.transform.root == visionHitArr[i].transform.root) continue;
            ...
            Vector3 forwardVision = visionT.forward;
            if (vision.head != null)
            {
                forwardVision.y = 0;
                if (forwardVision.sqrMagnitude > 0.0001f)
                {
                    forwardVision.Normalize();
                }
            }

            if (Vector3.Angle(forwardVision, (targetCaughtPoint - npcDir)) > (angle * 0.5f))
            {
            ...
            {
                if (targetCaughtPoint.y > (transform.position.y + vision.visionPosition.y + vision.maxSightLevel))
                {
                    enemiesTargetedByAmount.Remove(hostile);
                    continue;
                }
            ...
            if (vision.useMinLevel)
            {
                ...
                {
                    if (targetCaughtPoint.y < (transform.position.y + vision.visionPosition.y + vision.minSightLevel))
                    {
                        enemiesTargetedByAmount.Remove(hostile);
                        continue;
                    }
        ...
        if (ignoreUnreachableEnemy) 
        {
            if (!IsPathReachable(collOfTarget.transform.position)) 
            {
                ...
                if (ignoredEnemy != null) 
                {
                    ...
                    if (System.Array.IndexOf(visionHitArr, ignoredEnemy) >= 0) return;
                    ignoredEnemy = null;
                }
        ...
        if (vision.useVisionMeter) 
        {
            ...
            if (collOfTarget != null)
            {
                if (!CheckVisionMeter(true, collOfTarget.transform.gameObject, range)) return;
            }
        }
    ...
    public virtual void SawAlertTag(GameObject alertObj, int index)
    {
        ...
        // if behaviour is empty -> tell the user and exit
        if (vision.alertTags[index].behaviourScript == null) 
        {
            Debug.Log($"Alert Tag: {sawAlertTagName} behaviour is empty so no alert tag behaviour will be enabled on the AI.");
            return;
        }
    ...
    void SurroundingsCheck()
    {
        ...
        if (closeTarget.transform.root == transform.root) return;
        ...
        if (ignoreUnreachableEnemy) 
        {
            if (!IsPathReachable(closeTarget.transform.position))
            {
                ...
                if (ignoredEnemy != null)
                {
                    ...
                    if (closeTarget.transform.root == ignoredEnemy.transform.root) return;
                    ignoredEnemy = null;
                }
    ...
    GameObject CheckSurroundingForTarget()
    {
        ...
        int numColliders = Physics.OverlapSphereNonAlloc(transform.position + new Vector3(0, capsuleCollider.height/2, 0), enemyContactRadius, skinHitArr, vision.hostileAndAlertLayers);
    ...
    void ShowEnemyContactRadius()
    {
        ...
        Gizmos.DrawWireSphere(transform.position + new Vector3(0, capsuleCollider.height/2, 0), enemyContactRadius);
    }
    ...
    public void SetEnemy(GameObject enemy, bool turnStateToAttack = true, bool randomizePoint = false) 
    {
        ...
        if (state == State.attack || state == State.goingToCover) return;
    ...
    public virtual Vector3 RandomNavMeshLocation(float minLevel, float maxLevel, bool checkNotBehindObstacle = false) 
    {
        if (navmeshAgent == null)
        {
        ...
        for (int i=0; i<10; i++) 
        {
            ...
            point = hit.position;
            distance = CalculateCornersDistanceFrom(transform.position, point);
            ...
            if (checkNotBehindObstacle)
            {
                int layersToUse = vision.layersToDetect & ~groundLayers;
                if (Physics.Linecast(transform.position + new Vector3(0, navmeshAgent.height/2, 0), point, layersToUse))
                {
                    continue;
                }
            }
