...
namespace PolymindGames
{
    ...
    public sealed class StaminaManager : CharacterBehaviour, IStaminaManagerCC, ISaveableComponent
    {
        ...
        /// <summary>
        /// MRM-9 addition. Mr. Moonlight's no-hardcoded-values rule puts every gameplay number in
        /// MoonlightTunables, but the per-state stamina costs are private serialized fields on this
        /// component, so <c>MoonlightPlayerRig</c> needs a way to push the two knobs Carlos asked to
        /// have exposed (jump cost, sprint drain) in at startup.
        ///
        /// <para>Only the one-off enter cost and the per-second rate are settable, and only for a
        /// state that already exists in <c>_staminaStates</c> - this deliberately cannot add states
        /// or change the tuning shape Carlos signed off on, just retune it.</para>
        /// </summary>
        /// <returns>False if this character has no stamina state of that type configured.</returns>
        public bool TrySetStateCosts(MovementStateType stateType, float? enterChange, float? changeRatePerSec)
        {
            foreach (var state in _staminaStates)
            {
                ...
                if (enterChange.HasValue)
                    state.EnterChange = Mathf.Clamp(enterChange.Value, -1f, 1f);

                if (changeRatePerSec.HasValue)
                    state.ChangeRatePerSec = Mathf.Clamp(changeRatePerSec.Value, -1f, 1f);
