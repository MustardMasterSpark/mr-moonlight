...
namespace PolymindGames.SurfaceSystem
{
    ...
    public sealed partial class SurfaceManager : Manager<SurfaceManager>
    {
        ...
        [SerializeField, Title("Decals")]
        [Tooltip("MRM-9: master switch for bullet-impact decals. Off for now at Carlos's request; " +
                 "turn back on when the decal look is being worked on. Audio and visual impact " +
                 "effects are unaffected.")]
        private bool _decalsEnabled = false;
        ...
        public void PlayEffect(SurfaceEffectData effectData, in Vector3 position, in Quaternion rotation, SurfaceEffectPlayFlags flags, float volumeMultiplier = 1f, Transform parent = null)
        {
            ...
            // MRM-9: Carlos asked for impact decals to be switched off for now ("For now let's
            // remove any impact decals. We will do that later."). Gated here rather than by clearing
            // every SurfaceDefinition's DecalEffect reference, so turning them back on is one
            // checkbox and no asset data is lost.
            if (_decalsEnabled
                && (flags & SurfaceEffectPlayFlags.Decal) != 0
                && PoolManager.Instance.TryGet(effectData.DecalEffect, out var decalEffectInstance))
            {
