...
namespace PampelGames.GoreSimulator
{
    ...
    public class SubModuleCutRagdoll : SubModuleBase
    {
        ...
        internal void ExecuteRagdollSubModule(GameObject detachedObject, Mesh mesh, CutIndexClass cutIndexClass, BonesClass bonesClass,
            Vector3 cutCenter, SubModuleClass subModuleClass, GoreMultiCut goreMultiCut)
        {
            ...
            for (var i = 0; i < cutIndexClass.nonActiveBones.Count; i++)
            {
            ...
            {
                detachedGoreBones[i]._rigidbody.linearDamping = drag;
                detachedGoreBones[i]._rigidbody.angularDamping = angularDrag;
            ...
            subModuleObjClass.cutCenters = new List<Vector3> {cutCenter};
