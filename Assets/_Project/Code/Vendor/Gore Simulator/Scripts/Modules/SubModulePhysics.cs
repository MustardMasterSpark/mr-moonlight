...
namespace PampelGames.GoreSimulator
{
    ...
    public class SubModulePhysics : SubModuleBase
    {
        ...
        {
            ExecuteModuleInternal(subModuleClass);
            if (Application.isPlaying) ExecuteModuleInternalChildren(subModuleClass);
            ...
            for (var i = 0; i < subModuleClass.subModuleObjectClasses.Count; i++)
            {
                ...
                if(rigid.linearVelocity != Vector3.zero) rigid.linearVelocity = Vector3.zero;
                if(rigid.angularVelocity != Vector3.zero) rigid.angularVelocity = Vector3.zero;
                
                if(!subModuleClass.cachedPartsUsed)
                {
        ...
        private void ExecuteInternal(SubModuleObjectClass subModuleObjectClass)
        {
            ...
            {
                if (collider is Enums.Collider.Capsule ||
                    (collider == Enums.Collider.Automatic && fallbackCollider == Enums.FallbackCollider.Capsule))
                {
                    var capsuleCollider = obj.AddComponent<CapsuleCollider>();
                    MatchCapsuleColliderToBounds(mesh, boundsSize, capsuleCollider);
                    capsuleCollider.sharedMaterial = _goreSimulator.physicMaterial;
                }
                else
                {
                    var boxCollider = obj.AddComponent<BoxCollider>();
                    MatchBoxColliderToBounds(mesh, boundsSize, boxCollider);
                    boxCollider.sharedMaterial = _goreSimulator.physicMaterial;
                }
            ...
            if (collider == Enums.Collider.Box || (collider == Enums.Collider.Automatic && vertexCount > vertexLimit &&
                                                   fallbackCollider == Enums.FallbackCollider.Box))
            {
                var boxCollider = obj.AddComponent<BoxCollider>();
                boxCollider.sharedMaterial = _goreSimulator.physicMaterial;
            }
            else if (collider == Enums.Collider.Capsule || (collider == Enums.Collider.Automatic && vertexCount > vertexLimit &&
                                                            fallbackCollider == Enums.FallbackCollider.Capsule))
            {
                var capsuleCollider = obj.AddComponent<CapsuleCollider>();
            ...
            else if (collider is Enums.Collider.Mesh or Enums.Collider.Automatic)
            {
                ...
                meshCollider.sharedMaterial = _goreSimulator.physicMaterial;
            }
        }

        private void ExecuteInternalChild(GameObject child, Mesh sharedMesh)
        {
            ...
            if (collider != Enums.Collider.None && !child.TryGetComponent<Collider>(out var col))
            {
                                                                ...
                                                                fallbackCollider == Enums.FallbackCollider.Capsule))
                {
                    ...
                    if (sharedMesh != null) PGMeshUtility.MatchCapsuleColliderToBounds(sharedMesh, capsuleCollider);
                    capsuleCollider.sharedMaterial = _goreSimulator.physicMaterial;
        ...
        private void SetRigidbodyValues(Rigidbody rigid)
        {
            rigid.linearDamping = drag;
            rigid.angularDamping = angularDrag;
        }
        ...
        private void MatchCapsuleColliderToBounds(Mesh mesh, Vector3 boundsSize, CapsuleCollider capsuleCollider)
        {
            ...
            var division = vertices.Count / 4; // Only 4, less expensive.
            for (var j = 0; j < 4; j++)
            {
                var index = division * j;
